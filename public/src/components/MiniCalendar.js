...
function MiniCalendar({ monthStart, days, allDoneDays }) {
  ...
  return (
    <div className="rounded-xl bg-white p-4 text-slate-900 shadow-sm border border-slate-200">
      <div className="mb-3 text-center text-lg font-semibold tracking-wide">{MONTHS_FULL[first.getMonth()]} {first.getFullYear()}</div>
      <div className="grid grid-cols-7 gap-2 text-center text-xs font-medium uppercase text-slate-400">{"M T W T F S S".split(" ").map((l, i) => <div key={i}>{l}</div>)}</div>
        ...
        {cells.map((d, i) => {
          ...
          return (
            <div key={i} className="flex items-center justify-center h-9">
              {d ? (
                <div className={["flex h-8 w-8 items-center justify-center rounded-full text-sm font-medium transition-colors", done ? "bg-primary-600 text-white" : "bg-slate-100 text-slate-700", isTod ? "ring-2 ring-primary-500 ring-offset-1" : ""].join(" ")}>{d.getDate()}</div>
